...
class MultiResFeatureGrid2D(nn.Module):
    ...
    def __init__(
        ...
        self.output_dim = self.feature_dim  # Residual fusion: output dim matches feature dim
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        for lvl, r in enumerate(self._resolutions):
            ...
            f_accum = f_accum + f.to(dtype=grid.dtype)

        return f_accum


# ---------- Local Positional Encoding (future work) ----------
...
class LocalPosEnc2D(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        if self.qat_enabled:
            # Reusing the asymmetric quantize helper
