"""
V-NTBC Endpoint Network for BC4 compression (multi-single-channel-texture capable).
...
class LocalPosEnc2D(nn.Module):
    ...
    Notes:
    - Parameter name includes `grids` so Model_param_compress quantizes it.
    - Stores N/n_freq/d0 as buffers for inference auto-detect.
    """
...
class EndpointNetwork(nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        if self.lpe is not None:
            feats = torch.cat([feats, self.lpe(coords)], dim=1)
...
def endpoint_loss_bc4(
    ...
    # Build BC4 palette from PREDICTED endpoints (for selector assignment)
    ...
    # Build BC4 palette from REFERENCE endpoints (stable decode target)
    pal_ref = bc4_palette_bc4order(ref_endpoints2)  # (B,8)

    # Decode using REFERENCE palette (matches BC1 approach)
    # Hard decode
    decoded_hard = torch.gather(pal_ref, 1, hard_n.long())  # (B,16)

    # Soft decode (gradients flow through p -> dn -> pal_pred -> pred_endpoints)
    p = F.softmax(dn / float(temperature), dim=-1)  # (B,16,8)
    decoded_soft = (p * pal_ref.unsqueeze(1)).sum(dim=-1)  # (B,16)
...
if __name__ == "__main__":
    print("Quick test: V-NTBC BC4 EndpointNetwork forward + loss shapes")
    ...
    # Test VBQ
    net.encoding.enable_qat(bits_list=[8, 8, 8, 8, 8, 8, 8])
    pred2 = net(coords)
